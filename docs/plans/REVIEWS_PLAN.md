...
- AC-1.1: `store.New(filepath.Join(t.TempDir(), "neo.db"))` returns no error, creates the file, and the `reviews` table
  exists (asserted through `sqlite_master`).
- AC-1.2: `store.New("")` returns an error and creates no file.
- AC-1.3: `store.New` creates a missing parent directory: a path two levels inside a fresh temp directory opens
  successfully and the directory exists afterwards.
- AC-1.4: `store.New` on a path whose parent component is an existing regular file fails with a wrapped error and does
  not panic.
- AC-1.5: Opening the same path twice (close, reopen) succeeds and preserves data; the schema statements are idempotent.
- AC-1.6: After `Close`, further use of the client returns an error rather than panicking.
- AC-1.7: `dsn("/tmp/neo.db")` contains the path followed by `?_pragma=busy_timeout(5000)&_pragma=journal_mode(WAL)`,
  and contains no `file:` prefix.
- AC-1.8: The database is in WAL mode: after `New`, a raw `database/sql` connection opened by the test against the same
  file reports `wal` for `PRAGMA journal_mode`.
- AC-1.9: `Load()` reads `DB_PATH`, and the default is `neo-mcp.db` when the variable is unset.
- AC-1.10: `server.New` with `DBPath` in a fresh temp directory returns no error, creates the file, and logs
  `database opened` with that path.
- AC-1.11: `server.New` with an unopenable `DBPath` returns an error containing `open database`.
- AC-1.12: After `Server.Shutdown`, the store is closed: a subsequent query on `srv.store` fails.
- AC-1.13: `internal/server/server_test.go` sets `DBPath` to a temp path and no test creates `neo-mcp.db` in the
  repository working directory.
- AC-1.14: `.gitignore` ignores `*.db`, `*.db-wal`, and `*.db-shm`.
...
- AC-2.1: `AddReview` returns a positive id and a `CreatedAt` within a minute of now, and `Reviews` returns the same
  model, rating, and comment.
- AC-2.2: Ids increase monotonically, and deleting the newest review and adding another does not reuse the deleted id.
- AC-2.3: Ratings 0 and 11 are rejected with an error naming the rating and no row is written; ratings 1 and 10 are
  accepted.
- AC-2.4: An empty or whitespace-only model or comment is rejected; surrounding whitespace is trimmed before storage,
  so `"  cat  "` is stored as `cat`.
- AC-2.5: A comment containing `\n`, `\r`, a tab, or any other control character is rejected and nothing is written.
- AC-2.6: A comment of exactly 500 runes is accepted and one of 501 is rejected; a 500-rune comment of multi-byte
  characters is accepted, proving the limit counts runes, not bytes. The same check applies to a 200-rune model.
- AC-2.7: `Reviews` on an empty database returns an empty, non-nil slice and no error.
- AC-2.8: `Reviews` orders by model case-insensitively then by id ascending; a fixture with models `B`, `a`, and `A`
  returns `a`, `A`, then `B`, with each model's reviews in insertion order.
- AC-2.9: `DeleteReview` returns the deleted review, and deleting the same id again returns an error satisfying
  `errors.Is(err, store.ErrReviewNotFound)`.
- AC-2.10: After `DeleteReview`, `Reviews` no longer contains the row and every other row is unchanged.
- AC-2.11: `DeleteReview(0)` and a negative id return `ErrReviewNotFound` and delete nothing.
- AC-2.12: Eight goroutines adding reviews concurrently all succeed and all rows are present when the test runs with
  `-race`, proving a single connection plus the busy timeout handles concurrent tool calls.
- AC-2.13: Every query is parameterised; no SQL string is assembled from the model, comment, or id.
- AC-2.14: Tests create the database under `t.TempDir()` and close the client in `t.Cleanup`, so no test writes inside
...
- AC-3.1: `New(Deps{})` returns a server and registers no tools.
- AC-3.2: `New(Deps{Forge: x})`, `New(Deps{NovelAI: y})`, and `New(Deps{OpenAI: z})` register exactly the tool sets they
  registered before this unit (`txt2img`/`img2img`/`bgkill`, `txt2img`/`img2img`/`anlas`, and `img2txt` respectively).
- AC-3.3: `Deps.Store` is reachable from the handlers (`handlers.store` equals the value passed).
- AC-3.4: `go test ./...` passes with no test expectations changed other than the constructor call shape.
- AC-3.5: `internal/server/server.go` builds `Deps` from config and still returns `build mcp server: %w` on failure.
- AC-3.6: `Deps` is taken and copied by value, so a caller cannot mutate the server's configuration after construction.
...
- AC-4.1: With `Deps.Store` set, listing tools over an in-memory session returns `add_review`, `get_reviews`, and
  `delete_review` in addition to the existing tools; with `Deps.Store` nil, none of the three is present.
- AC-4.2: `add_review` requires `model`, `rating`, and `comment`, with no defaults; `rating` reports `minimum` 1 and
  `maximum` 10; `model` reports `maxLength` 200 and `comment` `maxLength` 500.
- AC-4.3: `get_reviews` has no input properties, and `delete_review` requires `id` with no default.
- AC-4.4: A `CallTool` for `add_review` returns the new id in both the text and the structured content, and the row is
  visible to a following `get_reviews`.
- AC-4.5: A rating of 0 or 11 returns a tool error whose message contains `add_review`, and the table is unchanged.
- AC-4.6: `get_reviews` Markdown matches the format above for a fixture with two models, three reviews, and mixed cases,
  including the overall and per-model counts and averages.
- AC-4.7: On an empty database `get_reviews` returns `No reviews yet.`, `count` 0, and `average` 0.
- AC-4.8: `delete_review` for an existing id removes the row and returns the deleted model, rating, and comment; for an
  unknown id it returns a tool error containing the id and `not found`, and the table is unchanged.
- AC-4.9: `reviewsMarkdown` is tested directly with hand-built slices covering an empty slice, one review, several
...
- AC-4.10: Every tool logs a `tool called` debug entry naming the tool, and `add_review` and `delete_review` log the
  affected id at info level.
- AC-4.11: An end-to-end test with a real store on `t.TempDir()` adds two reviews for different models, lists them,
  deletes one by id, lists again, and asserts the Markdown reflects each step.
- AC-4.12: A store error (a closed client) surfaces as a tool error prefixed with the tool name, not a panic.
- AC-4.13: `server.New` passes the opened store into `Deps`, so a deployed server registers the review tools (asserted by
  AC-1.10 plus AC-4.1).
...
- AC-5.1: The README features list gains one bullet describing the three review tools, that ratings are 1-10 with one
  line of commentary, and that they are stored in a local SQLite database.
- AC-5.2: The README mentions `DB_PATH`, that the database is created automatically, and that Docker users should mount a
  volume at `/data` to keep it; the README stays high level and defers detail to `.env.example`.
- AC-5.3: `.env.example` lists `DB_PATH` under a `database` heading with its default and a note that the container path
  is `/data/neo-mcp.db`.
- AC-5.4: `docs/PROMPT.md` gains a short bullet telling the agent to check `get_reviews` before choosing between models
...
- HV-4: Build the image (`docker build .`) and run it with a named volume mounted at `/data` and no `DB_PATH` set;
  confirm it starts, the tools work, and the reviews survive replacing the container.
- HV-5: `.env.example` may be unreadable and unwritable for agents under editor privacy settings. If so, the human must
  apply AC-5.3 by hand.
